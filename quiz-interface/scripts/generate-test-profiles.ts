...
function generateAnswersForProfile(targetProfile: string): { answers: any[]; scores: Record<Axis, number>; percentages: Record<Axis, number> } {
  ...
  if (difficultProfiles.includes(targetProfile)) {
    ...
    console.log(`  ⚠️ No exact match found for ${targetProfile}, using greedy approach`);
  }
...
for (const targetProfile of targetProfiles) {
  ...
  results.push({
    ...
    match: targetProfile === actualProfile ? '✓' : '✗',
    D: percentages.D,
    I: percentages.I,
    S: percentages.S,
    C: percentages.C
...
results.forEach(r => {
  console.log(
    `  ${r.target.padEnd(4)} | ${r.actual.padEnd(6)} |   ${r.match}   | ${String(r.D).padStart(3)} | ${String(r.I).padStart(3)} | ${String(r.S).padStart(3)} | ${String(r.C).padStart(3)} |`
  );
});
