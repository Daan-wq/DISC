...
for (const profile of profiles) {
  ...
  if (!profileData) {
    console.log(`  ⚠️ No test data found for ${profile}, skipping`);
    continue;
  }
  ...
  if (!fs.existsSync(templateDir)) {
    console.log(`  ⚠️ Template not found for ${profile}, skipping`);
    continue;
  }
  ...
  if (!fs.existsSync(publicationPath)) {
    console.log(`  ⚠️ publication.html not found for ${profile}`);
    continue;
  }
