...
/**
 * Generate a signed URL for PDF download (admin only)
 * Returns a temporary download link valid for 1 hour
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.storage
      .from('quiz-docs')
      .createSignedUrl(attempt.pdf_path, 3600) // 1 hour in seconds
