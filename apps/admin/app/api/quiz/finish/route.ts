...
// LEGACY OVERLAY REMOVED: generateReportPdf no longer exists
// PDF generation now uses API2PDF in /api/rapport/download-pdf route
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // This route no longer generates PDFs inline.
    // Use /api/rapport/download-pdf to generate PDFs via API2PDF.
    // Mark attempt as ready for PDF generation and return redirect info.

    console.log(`[finish][${requestId}] Marking attempt as ready for API2PDF generation`)

    // Store result_payload for later PDF generation
    const resultPayload = {
      profileCode,
      percentages: finalPD?.results ? {
        natural: {
      ...
      candidateName: finalPD?.candidate?.full_name || 'Gebruiker',
    }
    ...
    console.log(`[finish][${requestId}] Attempt marked as pending, elapsed: ${timings.t_render - timings.t_start}ms`)

    // Finalize attempt
    await supabaseAdmin
      .from('quiz_attempts')
      .update({
        pdf_status: 'pending', // Ready for API2PDF download
        finished_at: new Date().toISOString(),
        processing_token: null,
        pdf_error: null,
        alert: hasAlert,
    ...
    console.log(`[finish][${requestId}] SUCCESS - Quiz finished, ready for PDF generation. Total: ${timings.t_total}ms`)
    console.log(`=== /api/quiz/finish END (SUCCESS) [${requestId}] ===`)

    return NextResponse.json({
      ok: true,
      message: 'Quiz finished successfully. PDF can be generated via the download route.',
      timings
    })
  } catch (e: any) {
